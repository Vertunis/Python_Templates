...
MODELS_FILE_PATH = os.path.join(MODELS_DIR, "mnist_cnn3.h5")  # h5 Datei wird von Keras zum speichern der Gewichte verwendet

if not os.path.exists(MODELS_DIR):
    os.mkdir(MODELS_DIR)

LOGS_DIR = os.path.abspath(r"M:\Google Drive\Programming\Python\01_Tutorials\Udemy Kurs Deep Learning\logs")
MODEL_LOG_DIR = os.path.join(LOGS_DIR, "mnist_cnn3")  # Muss einzigartiger name sein
...
def build_model(img_shape: Tuple[int, int, int], num_classes: int) -> Sequential:
    ...
    model.add(Conv2D(filters=32, kernel_size=3, padding="same", strides=1, input_shape=img_shape))  # Filter + kernel  muss bei Conv2D immer angegeben werden
    model.add(Activation("relu"))
    model.add(Conv2D(filters=32, kernel_size=3, padding="same"))  # Filter + kernel  muss bei Conv2D immer angegeben werden
    model.add(Activation("relu"))
    model.add(MaxPooling2D()) # Reduzierung der Komplexität des bildes -> 3 Parameter: Poolsize(Größe des Poolings); Strides = 2

    model.add(Conv2D(filters=64, kernel_size=3))  # Filter + kernel  muss bei Conv2D immer angegeben werden
    model.add(Activation("relu"))
    model.add(Conv2D(filters=64, kernel_size=3))  # Filter + kernel  muss bei Conv2D immer angegeben werden
    model.add(Activation("relu"))
    model.add(MaxPooling2D()) # Reduzierung der Komplexität des bildes -> 3 Parameter: Poolsize(Größe des Poolings); Strides = 2
